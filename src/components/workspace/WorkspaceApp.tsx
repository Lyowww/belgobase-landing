...
const text = {
  nl: {
    ...
    email: "E-mailadres", license: "Licentiecode (optioneel)", enrollmentLicense: "BelgoBase-licentiecode", licenseHelp: "Heb je al een BelgoBase-account? Dan volstaat je geregistreerde e-mailadres.", remember: "Op dit apparaat aangemeld blijven", send: "Code per e-mail ontvangen", working: "Even geduld…",
  ...
  en: {
    ...
    email: "Email address", license: "License code (optional)", enrollmentLicense: "BelgoBase license code", licenseHelp: "Already have a BelgoBase account? Your registered email address is enough.", remember: "Keep me signed in on this device", send: "Send email code", working: "Please wait…",
  ...
  fr: {
    ...
    email: "Adresse e-mail", license: "Code de licence (facultatif)", enrollmentLicense: "Code de licence BelgoBase", licenseHelp: "Vous avez déjà un compte BelgoBase ? Votre adresse e-mail enregistrée suffit.", remember: "Rester connecté sur cet appareil", send: "Recevoir un code par e-mail", working: "Un instant…",
...
export function WorkspaceApp({ locale }: { locale: Locale }) {
  ...
  return <main className={styles.page} lang={shellLanguage}><section className={styles.card} aria-labelledby="app-title">
    ...
    {phase === "profile" ? <>
      ...
      <form aria-busy={busy} className={styles.form} onSubmit={autofill}><label>{t.enterprise}<input autoComplete="off" inputMode="text" placeholder="BE 0123.456.789" required value={enterprise} onChange={(event) => { setEnterprise(event.target.value); setDetails(undefined); setDeclarations(initialDeclarations); }} /></label><button disabled={busy} type="submit">{busy ? t.working : t.find}</button></form>
