...
function desktopDownloadHref(value: unknown): string | undefined {
...
function accountReferences(result: Result): AccountReferences {
...
export function WorkspaceApp({ locale }: { locale: Locale }) {
  ...
  const loadSession = useCallback(async () => {
    ...
    if (response.ok && value.authenticated) { setAccount(value.account); setCsrf(value.csrf || ""); setError(""); setSessionRetryAvailable(false); setPhase("workspace"); return; }
    if (response.ok) { setAccount(undefined); setCsrf(""); setError(""); setSessionRetryAvailable(false); setPhase("login"); return; }
    if (response.status === 401) { setAccount(undefined); setCsrf(""); setPhase("login"); return; }
    if (response.status === 403 && ["license_inactive", "identity_changed", "authorization_denied"].includes(value.error || "")) {
      setAccount(undefined); setCsrf(""); setError(value.error || "license_inactive"); setPhase("login"); return;
    }
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      try {
        ...
        } else if (!cancelled && response.status === 403) {
          const result = await json(response);
          if (["license_inactive", "identity_changed", "authorization_denied"].includes(result.error || "")) {
            setAccount(undefined); setCsrf(""); setAccountOpen(false); setBrowserSessions([]); setReferences({});
  ...
  async function verifyLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.authenticated && result.csrf) { setAccount(result.account); setCsrf(result.csrf); setSessionRetryAvailable(false); setPhase("workspace"); }
      else {
        try { await loadSession(); }
  ...
  async function startEnrollment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setError("");
    try { const { response, result } = await request("/api/web/enrollment/start", { license_code: licenseCode.trim(), email: email.trim(), remember_browser: remember, language: shellLanguage }); if (!response.ok || !result.ok || !result.challenge_id) throw new Error(result.error || "unknown_error"); setChallengeId(result.challenge_id); setCode(""); setResendIn(30); setPhase("enrollCode"); } catch (reason) { setError(reason instanceof Error ? reason.message : "unknown_error"); } finally { setBusy(false); }
  }
  async function resendEnrollmentCode() {
    if (busy || resendIn > 0) return;
    setBusy(true); setError("");
    try { const { response, result } = await request("/api/web/enrollment/start", { license_code: licenseCode.trim(), email: email.trim(), remember_browser: remember, language: shellLanguage }); if (!response.ok || !result.ok || !result.challenge_id) throw new Error(result.error || "unknown_error"); setChallengeId(result.challenge_id); setCode(""); setResendIn(30); } catch (reason) { setError(reason instanceof Error ? reason.message : "unknown_error"); } finally { setBusy(false); }
  }
  ...
  async function complete(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      clearEnrollment();
      if (result.csrf) { setAccount(result.account); setCsrf(result.csrf); setPhase("workspace"); } else await loadSession();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "unknown_error"); } finally { setBusy(false); }
  }
  async function logout() {
    setBusy(true);
    try { const { response } = await request("/api/web/auth/logout", {}, csrf); if (!response.ok && response.status !== 401) throw new Error(); setAccount(undefined); setLicenseCode(""); setCsrf(""); setChallengeId(""); setCode(""); setBrowserSessions([]); setReferences({}); setCopyFeedback(undefined); clearEnrollment(); setError("signed_out"); setPhase("login"); } catch { setError("unknown_error"); } finally { setBusy(false); }
  }
  async function loadBrowserSessions() {
    setAccountBusy(true); setBrowserSessions([]); setError("");
    try {
    ...
    } catch (reason) { setBrowserSessions([]); setError(reason instanceof Error ? reason.message : "unknown_error"); } finally { setAccountBusy(false); }
  }
  ...
  return <main className={styles.page} lang={shellLanguage}><section className={styles.card} aria-labelledby="app-title">
    <p className={styles.brand}>BelgoBase</p>
    <div className={styles.formLanguage}><label><span className={styles.visuallyHidden}>Taal / Language / Langue</span><select aria-label="Taal / Language / Langue" value={shellLanguage} onChange={(event) => selectShellLanguage(event.target.value as ShellLanguage)}><option value="nl">NL</option><option value="fr">FR</option><option value="en">EN</option></select></label></div>
    {codePhase ? <><h1 id="app-title">{t.codeTitle}</h1><p className={styles.intro}>{phase === "loginCode" ? t.loginCodeHelp : t.enrollmentCodeHelp}</p>{error ? <p role="alert" className={styles.error}>{errorMessage(error, t)}</p> : null}<form aria-busy={busy} className={styles.form} onSubmit={phase === "loginCode" ? verifyLogin : verifyEnrollment}><label>{t.code}<input autoComplete="one-time-code" disabled={sessionRetryAvailable} inputMode="numeric" maxLength={12} required value={code} onChange={(event) => setCode(event.target.value)} /></label>{sessionRetryAvailable && phase === "loginCode" ? <button disabled={busy} type="button" onClick={() => void retrySession(true)}>{busy ? t.working : t.retrySession}</button> : <><button disabled={busy} type="submit">{busy ? t.working : phase === "loginCode" ? t.login : t.verify}</button><button className={styles.secondary} disabled={busy || resendIn > 0} type="button" onClick={() => void (phase === "loginCode" ? resendLoginCode() : resendEnrollmentCode())}>{resendIn > 0 ? t.resendIn.replace("{seconds}", String(resendIn)) : t.resend}</button></>}<button className={styles.secondary} disabled={busy} type="button" onClick={back}>{t.otherEmail}</button></form></> : null}
    {phase === "profile" ? <>
