...
MAX_CONCURRENT_ACCESS_TOKENS_PER_DEVICE = 2
...
def _validate_audit_metadata(action: str, metadata: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if action == "access_token_issued":
        if set(value) != {
            "revoked_prior_token_count",
            "max_concurrent_tokens_per_device",
        }:
            raise ValueError("Ongeldige device-auditmetadata.")
        revoked_count = value["revoked_prior_token_count"]
        maximum = value["max_concurrent_tokens_per_device"]
        if (
            not isinstance(revoked_count, int)
            or isinstance(revoked_count, bool)
            or revoked_count < 0
            or maximum != MAX_CONCURRENT_ACCESS_TOKENS_PER_DEVICE
        ):
            raise ValueError("Ongeldige device-auditmetadata.")
        return {
            "revoked_prior_token_count": revoked_count,
            "max_concurrent_tokens_per_device": maximum,
        }
...
def _issue_access_token_in_transaction(
    ...
    existing_valid_count = int(
        connection.execute(
            ...
            """,
            (device_row["device_id"], issued_at),
        ).fetchone()[0]
    )
    victim_count = max(
        0,
        existing_valid_count + 1 - MAX_CONCURRENT_ACCESS_TOKENS_PER_DEVICE,
    )
    if victim_count:
        cursor = connection.execute(
            ...
            WHERE token_hash IN (
                ...
                FROM device_access_tokens
                WHERE device_id = ? AND revoked_at IS NULL AND expires_at > ?
                ORDER BY issued_at ASC, expires_at ASC, token_hash ASC
                LIMIT ?
            )
              AND device_id = ? AND revoked_at IS NULL AND expires_at > ?
            """,
            (
                issued_at,
        ...
        if int(cursor.rowcount) != victim_count:
            raise RuntimeError("Access-tokenlimiet kon niet atomair worden toegepast.")
    connection.execute(
        """
    ...
    return token, expires_at, victim_count
...
def activate_device(
    ...
    try:
        license_digest = credential_hash(license_code, pepper_path)
    ...
    try:
        ...
        license_row = connection.execute(
            "SELECT * FROM licenses WHERE credential_hash = ? LIMIT 1", (license_digest,)
        ).fetchone()
        ...
        token, expires_at, revoked_prior_token_count = _issue_access_token_in_transaction(
        ...
        _write_audit(
            ...
            source_ip=source_ip,
            metadata={
                "revoked_prior_token_count": revoked_prior_token_count,
                "max_concurrent_tokens_per_device": MAX_CONCURRENT_ACCESS_TOKENS_PER_DEVICE,
            },
        )
        connection.commit()
        return {
    ...
    try:
        ...
        try:
            ...
            if row["license_expires_at"] and current_text >= row["license_expires_at"]:
                raise DeviceAuthError(GENERIC_DEVICE_ERROR)
            newest_valid = connection.execute(
                """
                SELECT token_hash
                FROM device_access_tokens
                WHERE device_id = ? AND revoked_at IS NULL AND expires_at > ?
                ORDER BY issued_at DESC, expires_at DESC, token_hash DESC
                LIMIT ?
                """,
                (
                    row["device_id"],
                    current_text,
                    MAX_CONCURRENT_ACCESS_TOKENS_PER_DEVICE,
                ),
            ).fetchall()
            if digest not in {str(item["token_hash"]) for item in newest_valid}:
                raise DeviceAuthError(GENERIC_DEVICE_ERROR)
            connection.execute(
